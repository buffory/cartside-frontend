...
const googleMapsApiKey = 'AIzaSyC9l575sdGSyATEFPKc3s4edWDZ_dQoFzs';

const RetailerMap = ({ list, krogerInfo = {}, walmartInfo = {} }) => {
    ...
    useEffect(() => {
        if (userLocation && mapRef.current) {
            setStoreTypes([...new Set(list.map(product => { return { name: product.retailer, key: product.retailer.toLowerCase() }}))])
    ...
    return (
            <div>
                ...
                { calcRoute ? (<div style={panelStyle}>
                ...
                </div>) : <button onClick={() => setCalcRoute(true)}>Find Route</button> }
            </div>
        );
}
